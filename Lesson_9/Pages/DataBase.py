...
class DataBase:
    """Эта функция создает словарь query, где 
    каждый ключ 
    предсавляет собой тип запроса
    к БД (создание, удаление компании), каждое значение -
    объект текстового зароса SQL."""
    query = {
    ...
    def create_company(self, company_name: str, description: str):
        try:
            ...
            print("[INFO] Error - can not work with SQL", _ex)
        finally:
            if connection:
                connection.close()
                print("[INFO] DB connection closed")

    # Удаляем компанию в базе данных
    def delete(self, company_id: int):
        try:
            with self.db.connect() as connection:
                connection.execute(self.query['delete_company'], parameters=dict(company_id))
                connection.commit()
        except Exception as _ex:
            print ("[INFO] Error - can not work with SQL", _ex)
        finally:
